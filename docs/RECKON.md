...
This is *mechanical* conflict detection. v0.2 does not understand that `"$49/mo"` and `"$50 per month"` are the same number stated differently, or that `"competitor priced low"` and `"competitor priced high"` are semantically opposed. Mechanical means: strict equality on primitive values. Semantic conflict reasoning is v0.4 (Level 2), powered by vector embeddings.
...
- **Object-valued shared keys do not produce conflicts in v0.2.** If two entries share a key whose values are both objects, no conflict surfaces even if the objects differ. Deferred to v0.4 with semantic detection.
